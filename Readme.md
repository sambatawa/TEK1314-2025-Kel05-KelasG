...
Pada proyek Internal Database Fortress, kami merancang dan membangun sebuah sistem keamanan untuk melindungi server database internal yang berfungsi sebagai penyimpanan data penting organisasi. Infrastruktur terdiri dari tiga node utama, yaitu Attacker Node (labVM Linux dari CyberOps), Target Database Server (Ubuntu Server dengan MySQL), dan Monitoring Node (Security Onion). Server database dijalankan pada Ubuntu Server dengan layanan MySQL (port 3306) serta akses administrasi melalui SSH (port 22). Karena sifatnya sebagai sistem internal, server ini menjadi target dari ancaman internal (insider threat) maupun pihak yang berhasil mengakses jaringan.

Untuk mensimulasikan ancaman tersebut, digunakan Attacker Node (labVM Linux dari CyberOps) yang melakukan aktivitas ICMP Ping (reconnaissance), Network scanning. Seluruh aktivitas jaringan dimonitor menggunakan Security Onion yang berfungsi sebagai sistem Intrusion Detection System (IDS) untuk merekam dan memonitor seluruh aktivitas jaringan yang mendeteksi potensi ancaman terhadap database server. Fokus pada fase ini adalah melakukan hardening terhadap server database dan memastikan bahwa seluruh aktivitas mencurigakan dapat terdeteksi oleh sistem monitoring sebelum memasuki fase serangan.
